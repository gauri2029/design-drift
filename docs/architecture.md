...
├── agents/       LangGraph runtime agents          (Design Analysis + Production Analysis + Supervisor built — Phase 3)
├── graph/        LangGraph graph definition/state   (Design Analysis + Production Analysis workflow built — Phase 3)
...
## Runtime multi-agent workflow (Design Analysis + Production Analysis vertical slices built)
...
The Supervisor, Design Analysis Agent, and Production Analysis Agent are
wired up so far (`app/graph/workflow.py`, `app/agents/`): `START ->
supervisor -> (design_analysis -> supervisor)* -> (production_analysis ->
supervisor)* -> END`, both exposed together via one `POST
/api/v1/projects/{project_id}/design-analysis` call (Production Analysis
today only covers the "screenshot" half of its planned tool set below —
DOM/computed-style extraction isn't built yet). The diagram below is the
