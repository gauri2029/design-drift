...
class Anchor(BaseModel):
    ...
    def matches(self, line: str) -> bool:
        ...
        Case-sensitive except for TEXT: ids and class names are
        ...
        capitalizes).
        """
        if self.kind is AnchorKind.TEXT:
...
def extract_anchors(
    *,
    ...
    for text in texts:
        collected.extend(
            Anchor(kind=AnchorKind.TEXT, value=literal)
...
def _from_html(html: str) -> list[Anchor]:
    ...
    return [
        ...
        *(Anchor(kind=AnchorKind.TEXT, value=value) for value in parser.texts),
...
def _is_usable(anchor: Anchor) -> bool:
    ...
    if anchor.kind in (AnchorKind.CLASS, AnchorKind.TEXT):
